...
int query_up(int x, int y)
{
     ...
     while(nxt[x] != nxt[y])
     {
          res += get(1, 1, n, tin[nxt[x]], tin[x]);
          x = parent[0][nxt[x]];
     }
     res += get(1, 1, n, tin[y], tin[x]); //use tin[y] to include Y
     return res;
}
...
void update_up(int x, int y, int val)
{
     while(nxt[x] != nxt[y])
     {
          upd(1, 1, n, tin[nxt[x]], tin[x], val);
          x = parent[0][nxt[x]];
     }
     upd(1, 1, n, tin[y], tin[x], val); //use tin[y] to include Y
}
